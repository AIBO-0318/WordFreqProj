...
- **탭3 · 비슷한 영화 추천** (콘텐츠 기반, **두 신호 모두 크롤링한 코멘트에서 추출**)
  - 🗣️ **분위기·소재**: 영화별 코멘트를 TF-IDF 벡터화 → 코사인 유사도
    - 코멘트 단어만으로 감독·시리즈·스튜디오 관계를 잡아냄
      (예: 기생충→봉준호 작품, 인터스텔라→놀란 작품, 어벤져스→마블 시리즈)
  - 😊 **관객 호불호 반응**: LSTM 감성모델(왓챠 코멘트로 학습)이 매긴 **영화별 호평률** 유사도
  - 최종점수 = `α·분위기 + (1-α)·반응유사`, 가중치 α를 슬라이더로 조절
  - 추천마다 "왜 비슷한지"(공통 키워드)와 분위기/반응/호평률을 함께 표시
